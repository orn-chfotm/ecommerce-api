...
- BackENd
  ...
  - Spring boot : 2.7.18 (GA) 최종 릴리스 버전
    - 2 버전대 채택 이유
      - Boot 2.7.x 버전대는 최대JDK 17 버전을 지원하는 프레임워크이다.
      - 아직 2.7.x 버전대를 사용하고 있는 프로젝트도 많이 존재하고 Spring Boot Security 설정이 3.x 버전으로 넘어가면서 방식이 변경되어 우선 2 버전대로 구성 후 3 버전대 마이그레이션을 도전할 계획이다.
    - 3 버전대 채택 이유
      - 최소 JDK 17버전 이상 지원
